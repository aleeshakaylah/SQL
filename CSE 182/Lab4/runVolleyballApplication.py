...
def teamPlayerCount(myConn, name):
    ...
    stmt = "SELECT COUNT(*) FROM Players pl, Teams t WHERE name = %s AND pl.teamid = t.teamid";
    ...
    return_string = 'Count of players on team ' + str(name) + ' is ' + str(teamCount)
...
def updateTeamCity(myConn, theTeamID, newTeamCity):
    ...
    try:
        ...
        print("Update for", theTeamID, "to", newTeamCity, "is bad", file=sys.stderr)
        myCursor.close()
        myConn.close()
        sys.exit(-1)

    return_string = 'Number of teams with teamID ' + str(theTeamID) + ' who moved to city ' + newTeamCity + ' is ' + str(k)
...
def main():
    ...
    print('-- Team Player Count Test --')
    print(teamPlayerCount(myConn, 'Birds'))
    print(teamPlayerCount(myConn, 'Skyscrapers'))
    print()
    # Perform the calls to updateTeamCity described in Section 6 of Lab4, and print their outputs.
    # You have to write this yourselves.
    print('-- Update Team City Test --')
    print(updateTeamCity(myConn, 202, 'Las Vegas'))
    print(updateTeamCity(myConn, 208, 'Toronto'))
    print()
    # Perform the calls to fireSomePlayers described in Section 6 of Lab4, and print their outputs.
    # You have to write this yourselves.
    print('-- Fire Some Players Test --')
    print('Number of players fired in fireSomePlayers when maxFired equals 2 is ' + str(fireSomePlayers(myConn, 2)))
    print('Number of players fired in fireSomePlayers when maxFired equals 3 is ' + str(fireSomePlayers(myConn, 3)))
